-- ---------- refund amount (for honest metrics) ----------
alter table public.applications add column refunded_amount numeric(10,2);
update public.applications set refunded_amount = paid_amount where payment_state = 'refunded' and refunded_amount is null;

drop function if exists public.apply_stripe_refund(text, text);
...
revoke execute on function public.apply_stripe_refund(text, text, numeric) from public, anon, authenticated;
grant execute on function public.apply_stripe_refund(text, text, numeric) to service_role;

-- ---------- age rule enforced in the schema (18 by tenancy start; not over 100) ----------
alter table public.applications
  add constraint applications_age_18 check ((tenant_dob + interval '18 years')::date <= tenancy_start),
  add constraint applications_age_plausible check ((tenant_dob + interval '100 years')::date >= tenancy_start);

-- ---------- create_referral: add the combined age + tenancy-range rules ----------
