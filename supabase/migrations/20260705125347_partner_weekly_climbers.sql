-- #5 Climber of the week for the weekly partner digest: within each partner, the
-- referrer whose fees-collected RANK improved the most this week vs the prior week
-- (same fees-primary basis as the leaderboard, item 12). Only referrers present in
-- BOTH windows are considered, so a clean rank delta exists; ties break by name.
...
begin
  ...
  prev as (
    select a.partner_id as pid, a.referrer_id as rid,
    ...
    from public.applications a join public.users u on u.id = a.referrer_id
    where a.referrer_id is not null and u.role <> 'superadmin'
    group by a.partner_id, a.referrer_id
  ),
  curr_r as (select pid, rid, nm, fees, sent, row_number() over (partition by pid order by fees desc, nm asc) as rnk from curr),
  prev_r as (select pid, rid, row_number() over (partition by pid order by fees desc) as rnk from prev),
  moved as (
