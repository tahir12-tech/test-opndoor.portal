...
Deno.serve(async (req) => {
  ...
  if (!STRIPE_SECRET.startsWith("sk_test_")) {
    return new Response("Test mode only (sk_test_ required).", { status: 400 });
  }
  ...
  if (insErr) {
    ...
    return new Response(`Could not record event: ${insErr.message}`, {
      status: 500,
    }); // let Stripe retry
  }
  ...
  try {
    if (event.type === "checkout.session.completed") {
      ...
      const appId =
        s.metadata?.application_id ??
        (typeof s.client_reference_id === "string"
      ...
      if (appId) {
        ...
        const { error: payErr } = await service.rpc(
        ...
        if (payErr) {
          ...
          throw new Error(
            `apply_stripe_payment failed: ${payErr.message}`
          );
        }
        ...
        const { data: appRow } = await service
          .from("applications")
          .select(
            "status, deed_state, guarantee_ref, tenant_title, tenant_last_name, tenant_email, prop_addr1, prop_postcode"
          )
          .eq("id", appId)
          .maybeSingle();
        ...
        if (appRow?.status === "paid" && !priorPaid?.length) {
          await service.from("activity_log").insert({
            ...
            message: `Guarantor fee paid (£${amount.toLocaleString(
              "en-GB"
            )}) via Stripe.`,
            actor: "Stripe",
          });
          ...
          if (appRow.tenant_email) {
            ...
            await deliverPaymentReceipt(service, {
              ...
              propertyAddr: [
                titleCaseAddress(appRow.prop_addr1),
                appRow.prop_postcode,
              ]
                .filter(Boolean)
                .join(", "),
              amount: amountGBP,
      ...
      if (pi) {
        ...
        const { data: appRow } = await service
          .from("applications")
          .select(
            "id, guarantee_ref, refund_after_start, tenant_title, tenant_last_name, tenant_email, prop_addr1, prop_postcode, pandadoc_document_id, deed_state"
          )
        ...
        if (appRow) {
          await service.from("activity_log").insert({
            ...
            kind: "refunded",
            message: "Payment refunded in Stripe.",
            actor: "Stripe",
          });
          ...
          if (appRow.refund_after_start) {
            await service.from("activity_log").insert({
              ...
              kind: "refund_anomaly",
              message:
                "POLICY ANOMALY: refunded on or after the tenancy start date, outside the refund policy. Review required.",
              actor: "System",
            });
          }
          ...
          const amountGBP = `£${refundAmount.toLocaleString("en-GB", {
            minimumFractionDigits: refundAmount % 1 === 0 ? 0 : 2,
            maximumFractionDigits: 2,
          })}`;
    ...
    try {
      await service.rpc("report_ops_incident", {
        p_type: "webhook_error",
        p_detail: `stripe-webhook ${event?.type ?? "?"}: ${msg}`,
      });
    } catch {
